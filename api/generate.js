...
- 각 제언은 "① 청유형 헤드라인"(원문자 번호 + 무엇을 하라는 ~하세요/~해보세요 요약) 한 줄 → 줄바꿈 → 설명(발견→원인→액션 통합) 순서. 항목은 ① ② ③ … 원문자 번호로.
...
2) 캠페인 목표에 맞는 지표인가? (인지도=CPM·도달 중심, 낮은 CTR·높은 CPC를 문제처럼 부각 안 함)
3) 이번 집행을 과지출로 암시했나? 다음 예산을 '줄이라/절반으로' 했나? → '기준선 삼아 확대'로.
4) 성과 신호에서 판촉·할인·이벤트 레버로 비약했나? → 같은 광고 레버(소재·지면·타겟)로.
...
export default async function handler(req, res) {
  ...
  try {
    ...
    const rec = await askAI(`위 규칙에 따라 보고서의 "4. 향후 제언" 한 섹션만 작성해. 인사이트 섹션이나 [운영 변경 이력] 같은 별도 진단 단락은 만들지 말고, 각 제언 항목(① ② ③ …) 안에 발견→원인→액션을 통합해 써. 보고서 3번(상세 성과) 그래프·표 수치를 그대로 재서술하지 마.`);
