...
- 카톡은 시간순으로 끝까지 읽고 '가장 마지막 결정(종료 시점의 최종 운영 상태)'을 반드시 반영해라 — 제언은 그 최종 상태를 기준으로 쓴다. 보고서 성과표는 기간 누적이라 초기에 예산을 많이 쓴 소재가 커 보일 수 있으니, 누적 표와 카톡의 최신 결정이 어긋나면 카톡의 최종 결정이 우선이다. 이미 중단하기로 한 소재·타겟·지면을 '계속 활용하세요'라고 쓰지 말고, 마지막에 채택한 것을 빠뜨리지 마라.
...
8) 카톡의 '마지막 결정(최종 운영 상태)'을 반영했나? 이미 중단하기로 한 소재·타겟을 '계속 하라'고 쓰지 않았나? 마지막에 채택한 것을 빠뜨리지 않았나?`;
